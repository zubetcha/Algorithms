...
function solution(id_list, report, k) {
  ...
  const filtered_report = Array.from(new Set(report))
  const report_id_list = filtered_report.map((report) => report.split(' ')[0])
  const reported_id_list = filtered_report.map((report) => report.split(' ')[1])
  const reported_count = countByArray(reported_id_list)

  for (let id in reported_count) {
    if (reported_count[id] >= k) {
      reported_id_list.map((reported_id, i) => {
        if (reported_id === id) {
          answer[id_list.indexOf(report_id_list[i])] += 1
        }
      })
    }
  }
...
// 한 유저가 동일한 유저에 대해 여러 번 신고한 경우를 거르기 위해 new Set을 이용해 중복 제거
