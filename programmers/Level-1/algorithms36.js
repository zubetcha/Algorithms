...
function solution(new_id) {
  ...
  const str = new_id.toLowerCase().replace(reg, '').replace(/\.+/g, '.').replace(/^\./, '').replace(/\.$/, '')
  const result = str === '' ? 'a' : str
  const _result = result.length > 15 ? result.substring(0, 15).replace(/\.$/, '') : result.replace(/\.$/, '')
  let answer = _result.length < 3 ? (_result.length == 1 ? _result + _result + _result : _result + _result.charAt(1)) : _result
  return answer
}
...
function solution(new_id) {
  ...
  const str = new_id
    ...
    .replace(/^\.|\.$/g, '')
  const result = str == '' ? 'a' : str
  const _result = result.length > 15 ? result.slice(0, 15).replace(/\.$/, '') : result
  let answer = ''
  if (_result.length < 3) {
    let lastStr = _result.charAt(_result.length - 1)
    answer = _result + lastStr.repeat(3 - _result.length)
  } else {
    answer = _result
  }
  return answer
